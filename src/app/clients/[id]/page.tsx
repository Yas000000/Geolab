...
interface BrandRow {
  ...
  delta: number | null; // percentage points vs. all prior qualifying runs, null if no prior data
}

function buildDashboardData(client: ClientData, platformFilter: string | undefined) {
  const completeRuns = client.runs.filter((r) => r.status === "COMPLETE");

  const matches = (r: RawResponseData) => !platformFilter || r.platform === platformFilter;

  const platformsAvailable = Array.from(
    new Set(completeRuns.flatMap((run) => run.rawResponses.map((r) => r.platform))),
  ).sort();

  // Per-run trend points -- runs with zero matching responses under the
  // current platform filter are skipped rather than plotted as a fake 0%.
  const trendPoints: TrendPoint[] = [];
  for (const run of completeRuns) {
    const matching = run.rawResponses.filter(matches);
  ...
  const qualifyingRuns = completeRuns.filter((run) => run.rawResponses.some(matches));
  const latestRun = qualifyingRuns[qualifyingRuns.length - 1];
  const priorRuns = qualifyingRuns.slice(0, -1);

  const allMatching = qualifyingRuns.flatMap((run) => run.rawResponses.filter(matches));
  const totalResponses = allMatching.length;
  const totalMentioned = allMatching.filter((r) => r.parsedResult?.clientBrandMentioned).length;
  const headlineVisibility = totalResponses ? (totalMentioned / totalResponses) * 100 : 0;

  const latestMatching = latestRun ? latestRun.rawResponses.filter(matches) : [];
  const priorMatching = priorRuns.flatMap((run) => run.rawResponses.filter(matches));

  const brandAcc = new Map<
    string,
    { name: string; isClientBrand: boolean; appearances: number; positions: number[]; latestCount: number; priorCount: number }
  >();

  function tally(responses: RawResponseData[], bucket: "latestCount" | "priorCount") {
      ...
      for (const m of r.parsedResult?.mentions ?? []) {
        ...
        if (existing) {
          ...
          brandAcc.set(key, {
            ...
            isClientBrand,
            appearances: 1,
            positions: m.position != null ? [m.position] : [],
            latestCount: bucket === "latestCount" ? 1 : 0,
            priorCount: bucket === "priorCount" ? 1 : 0,
          });
        }
      }
    }
  }
  tally(latestMatching, "latestCount");
  tally(priorMatching, "priorCount");

  const priorTotal = priorMatching.length;
  const latestTotal = latestMatching.length;
  ...
  const brandTable: BrandRow[] = Array.from(brandAcc.entries())
    .map(([key, b]) => {
      ...
      if (priorTotal > 0) {
        const latestPct = latestTotal ? (b.latestCount / latestTotal) * 100 : 0;
        const priorPct = (b.priorCount / priorTotal) * 100;
        delta = latestPct - priorPct;
      }
      return {
        key,
        name: b.name,
  ...
  return { platformsAvailable, trendPoints, headlineVisibility, totalResponses, brandTable };
}
...
export default async function ClientDetailPage({
  ...
  const { platformsAvailable, trendPoints, headlineVisibility, totalResponses, brandTable } =
    buildDashboardData(client, platformFilter);
      ...
      <div className="mx-auto max-w-5xl px-6 py-10">
        ...
        <section className="mb-8">
          <div className="mb-3 rounded-lg border border-zinc-200 bg-white px-4 py-3 dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <div className="mt-1 text-3xl font-semibold tabular-nums text-zinc-950 dark:text-zinc-50">
        ...
        <section className="mb-8">
          ...
          {brandTable.length === 0 ? (
            <div className="rounded-lg border border-zinc-200 bg-white px-4 py-10 text-center text-sm text-zinc-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-400">
              No completed runs yet{platformFilter ? " for this platform" : ""}.
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead className="bg-zinc-100 text-left text-xs uppercase tracking-wide text-zinc-500 dark:bg-zinc-900 dark:text-zinc-400">
                  <tr>
                    ...
                    <th className="px-4 py-2 font-medium tabular-nums">Change</th>
                  </tr>
                </thead>
